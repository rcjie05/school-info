...
// ── Session (loaded after DB function is defined) ─────────────────────────────
require_once __DIR__ . '/session.php';
startSecureSession();
autoCheckConcurrentSession();
...
function checkUserStatus() {
    ...
    if ($result->num_rows === 0) { $conn->close(); return false; }
    ...
    if ($db_token !== null && $sess_token !== null && !hash_equals($db_token, $sess_token)) {
        $conn->close(); destroySession();
        header('Location: ' . BASE_URL . '/login.html?error=session_displaced'); exit();
    }
    if ($user['status'] === 'inactive') {
        if ($user['deactivated_until']) {
            if (time() >= strtotime($user['deactivated_until'])) {
                $u = $conn->prepare("UPDATE users SET status='active', deactivated_until=NULL, deactivation_reason=NULL WHERE id=?");
...
function requireLogin() {
    if (!isLoggedIn()) { destroySession(); header('Location: ' . BASE_URL . '/login.html?error=session_expired'); exit(); }
    if (!checkUserStatus()) { destroySession(); header('Location: ' . BASE_URL . '/login.html?error=deactivated'); exit(); }
}

function requireRole($role) {
    requireLogin();
    if (!hasRole($role)) {
        error_log("[SECURITY] Unauthorized role access: user_id=" . ($_SESSION['user_id'] ?? 'unknown') . " tried role=$role");
        header('Location: ' . BASE_URL . '/login.html?error=unauthorized'); exit();
    }
}

function requireRoleApi($role) {
    header('Content-Type: application/json');
    if (!isLoggedIn()) { http_response_code(401); echo json_encode(['success'=>false,'message'=>'Session expired. Please log in again.']); exit(); }
    if (!checkUserStatus()) { http_response_code(403); echo json_encode(['success'=>false,'message'=>'Account is deactivated.']); exit(); }
    if (!hasRole($role)) { http_response_code(403); echo json_encode(['success'=>false,'message'=>'Unauthorized. Insufficient role.']); exit(); }
}
...
define('SMTP_FROM_NAME',  (function(){
    $c = getDBConnection();
    $r = $c ? $c->query("SELECT setting_value FROM system_settings WHERE setting_key='school_name' LIMIT 1") : false;
    $n = ($r && ($row = $r->fetch_assoc())) ? ($row['setting_value'] ?? 'School Portal') : 'School Portal';
    if ($c) $c->close();
    return $n;
})());
