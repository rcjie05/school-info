...
<head>
    ...
    <title>Leave Requests - Teacher</title>
...
async function loadMyLeaves() {
    ...
    hist.innerHTML = data.leaves.map(l => {
        ...
        const cancelBtn = l.status === 'pending' ? `<button onclick="cancelLeave(${l.id})" style="background:none;border:1px solid #ef4444;color:#ef4444;padding:0.3rem 0.75rem;border-radius:var(--radius-md);cursor:pointer;font-size:0.8rem;font-weight:600;margin-top:0.5rem;">Cancel</button>` : '';
...
async function submitLeave() {
    ...
    if (data.success) {
        ...
        document.getElementById('leaveType').value  = leaveTypes[0]?.id || '';
