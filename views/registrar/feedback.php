...
<head>
    ...
    <title>Feedback - Registrar Dashboard</title>
    ...
    <style>
        ...
        .filter-bar select, .filter-bar input { padding: 0.6rem 1rem; border: 1.5px solid #e5e7eb; border-radius: var(--radius-md); font-size: 0.9rem; font-family: inherit; background: white; }
        .filter-bar input { flex: 1; min-width: 180px; }
        .feedback-list { display: flex; flex-direction: column; gap: 1rem; padding: 1rem; }
        .feedback-item { background: var(--background-main); border-radius: var(--radius-md); padding: 1.25rem 1.5rem; cursor: pointer; border-left: 4px solid var(--primary-purple); transition: box-shadow 0.2s; }
        ...
        .response-box label { font-weight: 600; font-size: 0.875rem; display: block; margin-bottom: 0.5rem; }
        ...
        <header class="page-header">
            <div class="header-title">
                <button class="sidebar-toggle" id="sidebarToggle" aria-label="Toggle sidebar"><span></span><span></span><span></span></button>
                    <h1>Feedback</h1>
                <p class="page-subtitle">Review and respond to feedback from students and teachers</p>
            </div>
        </header>
...
<script>
let allFeedback = [], currentId = null;
...
async function loadFeedback() {
    const res = await fetch('../../api/registrar/get_feedback.php');
...
function filterFeedback() {
    ...
    const filtered = allFeedback.filter(f => {
        return (!search || f.subject.toLowerCase().includes(search) || f.submitted_by.toLowerCase().includes(search))
...
function renderFeedback(list) {
    ...
    if (!list.length) { container.innerHTML = `<div class="empty-state"><span>💬</span>No feedback found.</div>`; return; }
...
async function sendResponse() {
    ...
    const res = await fetch('../../api/registrar/respond_feedback.php', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ feedback_id: currentId, response, status: 'in_progress' })
    });
    const data = await res.json();
...
async function markResolved() {
    ...
    const res = await fetch('../../api/registrar/respond_feedback.php', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ feedback_id: currentId, status: 'resolved', response: response || null })
    });
    const data = await res.json();
...
function closeModal() { document.getElementById('feedbackModal').classList.remove('active'); currentId = null; }
