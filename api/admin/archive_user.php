...
$stmt = $conn->prepare("UPDATE users SET archived_at = NOW(), status = 'inactive' WHERE id = ?");
$stmt->bind_param("i", $user_id);

if ($stmt->execute()) {
    logAction($conn, $admin_id, "Archived user: {$user['name']} ({$user['email']}, {$user['role']})", 'users', $user_id);
    echo json_encode(['success' => true, 'message' => "User '{$user['name']}' has been archived"]);
} else {
