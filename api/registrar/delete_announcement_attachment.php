...
// Fetch attachment info — only allow deleting from announcements posted by this registrar
$stmt = $conn->prepare("
    SELECT aa.id, aa.file_path, aa.original_name, aa.announcement_id 
    FROM announcement_attachments aa
    JOIN announcements a ON a.id = aa.announcement_id
    WHERE aa.id = ? AND a.posted_by = ?
");
