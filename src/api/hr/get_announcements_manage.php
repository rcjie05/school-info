...
$sql = "
    SELECT
        a.id, a.title, a.content, a.target_audience, a.priority, a.posted_by,
...
while ($row = $result->fetch_assoc()) {
    // Mark if the HR user can edit/delete this announcement (own posts only)
    $row['can_edit'] = ($row['posted_by'] == $user_id);
...
echo json_encode(['success' => true, 'announcements' => $announcements, 'total' => count($announcements)]);
$conn->close();
?>
