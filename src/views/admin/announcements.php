...
<head>
    ...
    <style>
        ...
        .announcement-attachment img { max-width: 100%; max-height: 300px; border-radius: var(--radius-md); object-fit: contain; background: #f3f4f6; }
        .announcement-attachment video { max-width: 100%; max-height: 300px; border-radius: var(--radius-md); }
...
<body>
    ...
    <div id="announcementModal" class="modal">        <div class="modal-content">
        ...
        function renderAttachments(attachments) {
            ...
            return `<div class="announcement-attachment">` + attachments.map(att => {
                if (att.type === 'image') {
                    ...
                    return `<img src="${src}" alt="${att.original_name}" onclick="openLightbox('${src}')" style="cursor:zoom-in;">`;
                }
                if (att.type === 'video') {
                    const src = `../uploads/announcements/${att.path.split('/').pop()}`;
                ...
                const src = `../uploads/announcements/${att.path.split('/').pop()}`;
                return `<a class="file-link" href="${src}" target="_blank" download="${att.original_name}">${getFileIcon(att.original_name)} ${att.original_name}</a>`;
            }).join('') + `</div>`;
        }
        ...
        async function loadAnnouncements() {
            ...
            if (data.success) {
                ...
                data.announcements.forEach(a => {
                    ...
                    const attachmentsHtml = renderAttachments(a.attachments);
                    html += `
