...
<head>
    ...
    <link rel="apple-touch-icon" href="../../../public/images/logo2.jpg">
    <title>Faculty Directory - Admin Dashboard</title>
...
<body>
        ...
        <main class="main-content">
            <header class="page-header">
                <div class="header-title">
                    ...
                    <p class="page-subtitle">Teachers and registrar information</p>
                </div>
                <div class="header-actions">
                    <select id="deptFilter" onchange="loadFaculty()" style="padding: 0.5rem; border-radius: var(--radius-md); border: 1px solid #ddd;">
    ...
    <script>
        ...
        async function loadFaculty() {
            ...
            const response = await fetch(`../../api/admin/get_faculty.php${params}`);
            const data = await response.json();
            
            if (data.success) {
                // Update department filter if needed
                if (allDepartments.length === 0 && data.departments.length > 0) {
                ...
                let html = '<table class="data-table"><thead><tr><th>Name</th><th>Department</th><th>Email</th><th>Office Location</th><th>Office Hours</th><th>Status</th></tr></thead><tbody>';
                data.faculty.forEach(f => {
                    html += `<tr>
                        <td>${f.name}</td>
                        <td>${f.department || 'N/A'}</td>
                        <td>${f.email}</td>
                        <td>${f.office_location || 'N/A'}</td>
                        <td>${f.office_hours || 'N/A'}</td>
                        <td><span class="status-badge status-${f.status}">${f.status}</span></td>
                    </tr>`;
                });
                html += '</tbody></table>';
                document.getElementById('facultyTable').innerHTML = data.faculty.length > 0 ? html : '<p style="text-align: center; color: var(--text-secondary); padding: 2rem;">No faculty members found</p>';
            }
        }
