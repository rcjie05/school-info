...
$fullName = $_SESSION['name'] ?? 'HR Officer';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .zoom-btn:hover { background: #f8fafc; border-color: #2563eb; color: #2563eb; transform: scale(1.05); }
        .legend {
            background: #f8fafc; padding: 16px 20px; border-radius: 12px;
            border: 1px solid #e2e8f0; display: flex; flex-wrap: wrap; gap: 20px; align-items: center;
        }
        .legend h4 { margin: 0; font-size: 14px; font-weight: 600; color: #1e293b; display: flex; align-items: center; gap: 6px; }
        .legend-items { display: flex; flex-wrap: wrap; gap: 16px; flex: 1; }
        .legend-item { display: flex; align-items: center; gap: 8px; font-size: 13px; color: #475569; transition: all 0.2s; }
        .legend-item:hover { color: #1e293b; }
                    ...
                    <div class="right-panel">
                        <div class="saved-routes" id="hrRouteSelector">
                            <div class="control-section">
                                ...
                                <input type="text" class="input-field" id="hrRouteSearch"
                                    placeholder="🔍 Search routes..."
                                    oninput="filterHrRoutes()"
...
<script>
    window.currentUserRole = 'hr';
    window.canEditRoutes   = false;
    window.userFullName    = '<?php echo htmlspecialchars($fullName); ?>';

    // floor-script.js now handles hr/registrar/teacher/student IDs natively
</script>
<script src="../../../public/js/floor-script.js"></script>
<script>
    window.addEventListener('load', function() {
        const canvas = document.getElementById('floorPlan');
        if (canvas && typeof drawFloorPlan === 'function') {
            try { drawFloorPlan(); } catch(e) { console.error('Floor plan error:', e); }
        }
    });
</script>
<script>
