...
// putenv('GROQ_API_KEY=your_key_here'); // ← uncomment and add key for LOCAL only, never push to GitHub!

// Database configuration
// Supports Railway.app env variables and XAMPP local fallback
// putenv('GROQ_API_KEY=your_key_here'); // add your NEW key here for local use only
...
function checkUserStatus() {
    ...
    if ($db_token !== null && $sess_token !== null && !hash_equals($db_token, $sess_token)) {
        $conn->close();
        destroySession();
        header('Location: ' . BASE_URL . '/public/login.html?error=session_displaced');
        exit();
    }
...
function requireLogin() {
    if (!isLoggedIn()) {
        ...
        destroySession();
        header('Location: ' . BASE_URL . '/public/login.html?error=session_expired');
        exit();
    }
    if (!checkUserStatus()) {
        destroySession();
        header('Location: ' . BASE_URL . '/public/login.html?error=deactivated');
        exit();
    }
}
...
function requireRole($role) {
    ...
    if (!hasRole($role)) {
        ...
        header('Location: ' . BASE_URL . '/public/login.html?error=unauthorized');
        exit();
    }
}
